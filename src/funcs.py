...
def initialize_grid(saved_params):
    ...
    skin_thickness =1.6*saved_params['patch_0']['a'][0]
    ...
    # ax.plot(saved_params['blade_nodes'][...,0].T[:,0],saved_params['blade_nodes'][...,-1].T[:,0])
    # # ax.plot(blade_nodes_offset[...,0].T[:,-1],blade_nodes_offset[...,-1].T[:,-1])
    # # ax.plot(af.coordinates[...,0],af.coordinates[...,1])
    # # ax.plot(blade_nodes[1,:,0].flatten(),blade_nodes[1,:,-1].flatten())
...
def arange_resonators(saved_params, uniform = True,preroute = True):
    ...
    N_res_types = saved_params['patch_0']['N'][:,saved_params['filt_ind']].astype(int)
    N_total = np.sum(N_res_types,axis = 0).squeeze()
    c_extents = saved_params['c_extents'][:,None]*saved_params['c']
    ...
    for i in range(np.sum(saved_params['filt_ind'])):
        ...
        grid_coord = np.array(list(map(lambda x:x[r_ind[:-1][saved_params['filt_ind']][i]:r_ind[1:][saved_params['filt_ind']][i]-1,c_ind[saved_params['filt_ind']][i][0]+1:c_ind[saved_params['filt_ind']][i][1]],[saved_params['x_mgrid'],saved_params['y_mgrid'],saved_params['z_s']]))).T
        grid = np.ones(grid_coord.shape[:2],dtype=bool)
        
        if np.prod(grid_coord.shape[:2])<N_total[i]:
            print(f'Not able to achieve the target OAR of {np.round(N_total[i]*np.pi*saved_params['patch_0']['a']**2/saved_params['A_s'][i],2)}. Will use an OAR of {np.round(np.prod(grid_coord.shape[:2])*np.pi*saved_params['patch_0']['a']**2/saved_params['A_s'][i],2)} instead. Decrease spatial resolution to achieve target.')
        ...
        if preroute:
            ...
            for y_iter in range(len(res_nodes_y)):
                ...
                for node_iter,node in enumerate(nodes):
                    ...
                    N_nodes = np.max((initial_depth-node_iter%len(nodes),1))
    ...
    # # ax.scatter(saved_params['grid_coord'][...,0][saved_params['grid']],saved_params['grid_coord'][...,1][saved_params['grid']],saved_params['grid_coord'][...,-1][saved_params['grid']])
        ...
        if (total_length-L)<=0:
            ...
            # if all adjescent points are occupied and a candidate directions doesn't exist aka dead end
                ...
                if len(closed_set)>3:    
                    ...
                    break
    if not packed:
        saved_params['grid'][get_index(np.asarray(closed_set)[1:],saved_params)] = True
        closed_set = []
    return closed_set
...
def route_resonators(saved_params,randomize = True):
    ...
    while N_remaining<N_starting:
        ...
        for i in range(N_starting):
            res_path = route(starting_node = saved_params['res_nodes'][route_order_ind[i]],total_length = saved_params['patch_0']['L'][saved_params['res_types'][route_order_ind[i]]],saved_params = saved_params)
            if len(res_path)>0:
